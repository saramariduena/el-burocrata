...
function ensureCtx(): Ctx | null {
  ...
  if (!ctx) {
    try {
      ...
      musicGain.gain.value = 0.06; // música bien suave, de fondo
...
// Toca un acorde suave y programa el siguiente (loop de música de fondo).
function scheduleChord(): void {
  ...
  chordIdx += 1;
  musicTimer = setTimeout(scheduleChord, dur * 900);
}
